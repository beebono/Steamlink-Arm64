...
# Pre-download library setup
GAMEDIR="/$directory/ports/steamlink"
export LD_LIBRARY_PATH="$GAMEDIR/libs.${DEVICE_ARCH}"
...
# Post-download library setup
QT_VERSION=$(ls -d $GAMEDIR/Qt-* 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -n1)
game_executable=./"bin/shell.${DEVICE_ARCH}"
game_libs="$GAMEDIR/Qt-${QT_VERSION}/lib:$LD_LIBRARY_PATH"
game_preload="$GAMEDIR/libs.${DEVICE_ARCH}/libgpucompat.so"
...
# Assign westonwrap and load the game
pm_platform_helper "$GAMEDIR/bin/shell.${DEVICE_ARCH} " >/dev/null
$ESUDO env WRAPPED_LIBRARY_PATH=$game_libs:$weston_dir/lib_aarch64 WRAPPED_PRELOAD=$game_preload \
$weston_dir/westonwrap.sh drm gl kiosk crusty_x11egl \
SDL_EGL_LIBRARY=libCRU.so SDL_VIDEO_FORCE_EGL=1 SDL_VIDEO_DRIVER=x11 $GAMEDIR/$game_executable
